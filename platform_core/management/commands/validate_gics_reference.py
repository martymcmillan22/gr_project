...
class Command(BaseCommand):
    help = "Validate licensed GICS CSV structure without importing data."
    ...
    def handle(self, *args, **options):
        ...
        total_rows = sum(level_counter.values())
        breakdown = ", ".join(
            [
                f"sector={level_counter[GICSReference.LEVEL_SECTOR]}",
                f"industry_group={level_counter[GICSReference.LEVEL_INDUSTRY_GROUP]}",
                f"industry={level_counter[GICSReference.LEVEL_INDUSTRY]}",
                f"sub_industry={level_counter[GICSReference.LEVEL_SUB_INDUSTRY]}",
            ]
        )
        self.stdout.write(self.style.SUCCESS(f"GICS CSV validation passed: rows={total_rows} ({breakdown})"))